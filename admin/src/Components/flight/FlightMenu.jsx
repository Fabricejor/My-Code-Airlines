...
export default function FlightMenu() {
  ...
  useEffect(() => {
    const fetchFlights = async () => {
      try {
        const response = await fetch("http://localhost:5000/api/allFlight");
      ...
      <Zoom className="flight-content">
        <table className="table-flight">
          <thead>
            <tr>
              <th>Compagnie</th>
              <th colSpan={2}>Trajets</th>
              <th className="date">Départ</th>
              <th className="date">Arrivée</th>
              <th>Distance</th>
              <th>Place</th>
              <th>Prix</th>
          ...
          <tbody>
            {flights.map((flight) => (
              <tr key={flight._id}>
                ...
                <td>{flight.airport_start}</td>
                <td>{flight.airport_end}</td>
                <td>{formatDate(flight.date_depart)}</td>
                <td>{formatDate(flight.date_arrivee)}</td>
                <td>{flight.distance} km</td>
                <td>{flight.place}</td>
                <td>{flight.prix} xof</td>
                <td>
                  <TbEdit />
                </td>
                <td>
